...
export async function calculateDynamicPrice(
  ...
  const totalAdjustmentPercent = Object.values(adjustments).reduce(
    (sum, adj) => sum + adj,
    0,
  );
  ...
  if (adjustments.season > 0)
    reasons.push(`High season (+${adjustments.season}%)`);
  if (adjustments.season < 0)
    reasons.push(`Low season (${adjustments.season}%)`);
  if (adjustments.weather > 0)
    reasons.push(`Ideal weather (+${adjustments.weather}%)`);
  if (adjustments.weather < 0)
    reasons.push(`Weather conditions (${adjustments.weather}%)`);
  if (adjustments.demand > 0)
    reasons.push(`High demand (+${adjustments.demand}%)`);
  if (adjustments.demand < 0)
    reasons.push(`Low demand (${adjustments.demand}%)`);
  if (adjustments.weekend > 0)
    reasons.push(`Weekend travel (+${adjustments.weekend}%)`);
  if (adjustments.lastMinute > 0)
    reasons.push(`Last-minute booking (+${adjustments.lastMinute}%)`);
...
export async function calculateMultiDestinationPrice(
  ...
  const totalBasePrice =
    destinations.reduce((sum, dest) => sum + dest.price, 0) * guests * nights;
  ...
  const avgAdjustments: PricingAdjustments = {
    season:
      results.reduce((sum, r) => sum + r.adjustments.season, 0) /
      results.length,
    weather:
      results.reduce((sum, r) => sum + r.adjustments.weather, 0) /
      results.length,
    demand:
      results.reduce((sum, r) => sum + r.adjustments.demand, 0) /
      results.length,
    weekend: results[0].adjustments.weekend, // Same for all
    lastMinute: results[0].adjustments.lastMinute, // Same for all
  };

  const totalAdjustmentPercent = Object.values(avgAdjustments).reduce(
    (sum, adj) => sum + adj,
    0,
  );
