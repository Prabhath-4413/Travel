...
public sealed class BookingQueueConsumerService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Booking queue consumer encountered an error.");
    ...
    private async Task EnsureConsumerAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Booking queue consumer subscribed to {Queue}.", _queueName);
        }
        finally
        {
    ...
    private async Task OnMessageReceived(object sender, BasicDeliverEventArgs args)
    {
        ...
        try
        {
            ...
            if (message == null)
            {
                _logger.LogWarning("Booking payload deserialized to null. MessageId {MessageId}.", messageId);
                _channel.BasicAck(args.DeliveryTag, false);
                return;
            }

            await ProcessBookingAsync(message, messageId, _stoppingToken);
            _channel.BasicAck(args.DeliveryTag, false);
            _logger.LogInformation("Booking message processed. MessageId {MessageId}.", messageId);
        }
        catch (JsonException ex)
        {
            _logger.LogWarning(ex, "Invalid booking payload. MessageId {MessageId}.", messageId);
            _channel.BasicAck(args.DeliveryTag, false);
        }
        catch (OperationCanceledException)
        {
    ...
    private async Task ProcessBookingAsync(BookingMessage message, string messageId, CancellationToken cancellationToken)
    {
        using var scope = _scopeFactory.CreateScope();
        var emailService = scope.ServiceProvider.GetService<IEmailService>();
        if (emailService == null)
        {
            _logger.LogWarning("IEmailService not resolved for booking message {MessageId}.", messageId);
            return;
        }
        ...
        var email = new Travel.Api.Services.EmailMessage
        {
            ...
            Body = EmailTemplates.BookingConfirmed(
                message.UserName,
                message.BookingId,
                string.Join(", ", message.Destinations),
                message.StartDate,
                message.StartDate.AddDays(message.Nights),
                message.Guests,
                message.TotalPrice),
            IsHtml = true
        };
    ...
    private async Task HandleFailureAsync(BasicDeliverEventArgs args, byte[] body, string messageId, Exception exception)
    {
        ...
        {
            _logger.LogError(exception, "Booking message failed permanently. MessageId {MessageId}.", messageId);
        ...
        _logger.LogWarning(exception, "Booking message retry {Retry}/{Max}. MessageId {MessageId}.", nextRetry, _maxRetries, messageId);
    ...
    private void SafeNack(BasicDeliverEventArgs args, bool requeue)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to negatively acknowledge booking message.");
        }
    }

    private void OnChannelShutdown(object? sender, RabbitMQ.Client.ShutdownEventArgs args)
    {
        _logger.LogWarning("Booking queue channel shutdown {Code} - {Text}.", args.ReplyCode, args.ReplyText);
    ...
    public Task CloseChannelAsync()
    {
        ...
        {
            _logger.LogWarning(ex, "Error closing booking queue channel.");
        }
        finally
        {
            _channel.Dispose();
